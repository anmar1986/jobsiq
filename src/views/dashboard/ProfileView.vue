  ...
  <div class="bg-gray-50 min-h-screen py-8">
    <div class="mx-auto px-4 sm:px-6 lg:px-8" style="max-width: 1200px;">
      ...
      <div class="mb-8 flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 mb-2">{{ $t('profile.myProfile') }}</h1>
          <p class="text-gray-600">{{ isEditMode ? $t('profile.updateProfile') : $t('profile.viewProfile') }}</p>
        </div>
        <div v-if="!isEditMode">
          <BaseButton
            variant="primary"
      ...
      <div v-if="!isEditMode">
        <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
          ...
          <div class="border-b border-gray-200 pb-6">
            <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.personalInformation') }}</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-if="authStore.user?.gender">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.gender') }}</p>
                <p class="text-gray-900 capitalize">{{ $t(`profile.${authStore.user.gender}`) }}</p>
              </div>
              <div v-if="authStore.user?.date_of_birth">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.dateOfBirth') }}</p>
                <p class="text-gray-900">{{ formatDate(authStore.user.date_of_birth) }}</p>
              </div>
              <div v-if="authStore.user?.nationality">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.nationality') }}</p>
                <p class="text-gray-900">{{ authStore.user.nationality }}</p>
              </div>
              <div v-if="authStore.user?.phone_number">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.phoneNumber') }}</p>
                <p class="text-gray-900">{{ authStore.user.phone_number }}</p>
              </div>
              <div v-if="authStore.user?.city">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.city') }}</p>
                <p class="text-gray-900">{{ authStore.user.city }}</p>
              </div>
              <div v-if="authStore.user?.country">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.country') }}</p>
                <p class="text-gray-900">{{ authStore.user.country }}</p>
              </div>
              <div v-if="authStore.user?.address" class="md:col-span-2">
                <p class="text-sm font-semibold text-gray-500">{{ $t('profile.streetAddress') }}</p>
          ...
          <div v-if="authStore.user?.linkedin_url">
            <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.socialLinks') }}</h3>
            <div>
              <a 
      ...
      <div v-else>
        <form @submit.prevent="handleSubmit" class="bg-white rounded-lg shadow-md p-6 space-y-6">
          ...
          <div class="border-b border-gray-200 pb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.profilePhoto') }}</h2>
            <div class="flex items-center gap-6">
              ...
              <div class="flex-1">
                ...
                <p class="text-sm text-gray-500">{{ $t('profile.photoRequirements') }}</p>
                <button
          ...
          <!-- Basic Information -->
          <div class="border-b border-gray-200 pb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.basicInformation') }}</h2>
          ...
          <!-- Personal Information -->
          <div class="border-b border-gray-200 pb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.personalInformation') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label for="profile-gender" class="block text-sm font-semibold text-gray-700 mb-2">{{ $t('profile.gender') }}</label>
                <select
                  id="profile-gender"
          ...
          <!-- Social Links -->
          <div class="pb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">{{ $t('profile.socialLinks') }}</h2>
            <BaseInput
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const errorMsg = errorObj?.response?.data?.errors 
      ...
      : errorObj?.response?.data?.message || t('profile.failedToUpdate')
